...
from data_translate.domain.text_processing.guards import has_letters
from data_translate.domain.text_processing.parsers import extract_quoted_value
...
def _extract_agent_say_utterance(record: str, *, agent_prefix: str) -> tuple[str, str, str] | None:
    ...
    parsed = extract_quoted_value(action, start_idx=value_start_idx)
    if parsed is None:
        return None
...
async def _translate_preserving_blank_lines(
    ...
    *,
    ...
    if not text.strip() or not has_letters(text):
        return text, 0, ""
