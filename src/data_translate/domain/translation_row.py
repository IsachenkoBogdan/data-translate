...
async def translate_by_rule(rule: TranslationRuleModel, row: dict[str, Any], adapter: TranslationAdapter) -> StrategyResult:
    ...
    value = row.get(rule.source)
    options = rule_options(rule)
    validate_rule_value(rule, value)
    if should_skip_translation(value, options):
        return StrategyResult(value=value, error="", attempts=0)
    strategy = STRATEGIES[rule.strategy]
    return await strategy(value, adapter, options, use_cache=rule.cache)
